// в файле ForecastPage в функции void ForecastPage::updateAllCells(DataResiver& dataresiver)
// разработать логику присвоения переменной(WeatherType cell_Weathertype;) значения погоды 
// эта переменная принадлежит структуре Cell принадлежащей классу ForecastPage
// Это значение приходит с сервера для 40 точек погоды с шагом каждые 3 часа
// А у меня 20 точек выводится на дисплей, поэтому надо продумать логику какое из двух значений приходящих
// c сервера присваивать той точке которая будет показываться на дисплее
// Температура выводится среднее между двумя значениями, а вот какое из значений погоды выводить на дисплей?
